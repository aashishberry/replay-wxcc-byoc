...
export function ensureSchema() {
  if (!initialized) {
    ...
    initialized = db.batch([
      ...
      db.prepare("CREATE INDEX IF NOT EXISTS tasks_updated_idx ON tasks(updated_at DESC)"),
      db.prepare("CREATE INDEX IF NOT EXISTS messages_task_idx ON messages(task_id, created_at ASC)"),
      db.prepare("CREATE INDEX IF NOT EXISTS events_task_idx ON events(task_id, created_at DESC)"),
    ]).then(() => undefined);
  }
  return initialized;
}
